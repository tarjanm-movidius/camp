...
void fl_showents( int startpos, struct filelistent *filelist )
{
    ...
    while ( (filelist->next != NULL) ) {
        if ( filelist->number == current[depth] ) /* active */
            if ( filelist->type == 2 ) printf("\e[%sm", config.skin.flistcda);
            else if ( !filelist->tagged ) printf("\e[%sm", config.skin.flistcfa);
            else
                printf("\e[%sm", config.skin.flistcta);
        else /* inactive */
            if ( filelist->type == 2 ) printf("\e[%sm", config.skin.flistcdi);
            else if ( !filelist->tagged ) printf("\e[%sm", config.skin.flistcfi);
            else
...
void togglemark( struct filelistent *filelist, char movedown )
{
    ...
    if ( filelist->tagged == FALSE ) filelist->tagged = TRUE;
    else
...
void fl_updatebuttons(int add)
{
    ...
    for (i=FL_MINBUTTON; i<(FL_MAXBUTTON+1); i++) {
        ...
        if (config.skin.flistbo[fl_buttonpos] == i) printf("%s", config.skin.fa[i]);
        else
    ...
    while ( ( direntptr = readdir(dirptr) ) != NULL ) {
        if (strcmp(direntptr->d_name, ".")) {
            if ( !strcmp(direntptr->d_name, "..") && !strcmp(dir, "/") )
                continue;
            else if ( config.hidedot && direntptr->d_name[0] == '.' && strcmp(direntptr->d_name, "..") )
                continue;
        ...
        while ( filelist->next != NULL ) {
            for(j=0; j<strlen(filelist->name); j++)
                if( filelist->type == 2 && temp->type == 1 ) {
                    temp = filelist;
                    break;
                } else if ( filelist->type == 1 && temp->type == 2 )
                    break;
                else if( filelist->name[j] < temp->name[j] ) {
                    temp = filelist;
                    break;
                } else if ( filelist->name[j] != temp->name[j] )
                    break;
...
void saveplaylist(struct playlistent *playlist, char *filename, char directsave)
{
    ...
    if ( directsave ) strcpy(buf2, filename);
    else {
...
#ifdef HAVE_LIBZ
            if ( config.compresspl )
                gzwrite(gzfd, (void*)&getpl, sizeof(struct oneplaylistent));
            else
...
#ifdef HAVE_LIBZ
    if ( config.compresspl && cplid3 ) gzclose(gzfd);
    else
        fclose(fd);
#else
    fclose(fd);
    ...
    if ( event.buttons & 4 && event.type & GPM_DOWN && !(event.type & GPM_DRAG) )
        if ( event.y == config.skin.flisty-1 && event.x >= config.skin.flistx && event.x < (config.skin.flistx+config.skin.flistw) ) {
            ...
            if ( current[depth] < screenmark[depth] )
                current[depth] = screenmark[depth];
            fl_showents(current[depth]-screenmark[depth], *filelist);
        } else if ( event.y == (config.skin.flisty+config.skin.flistlines) && event.x > config.skin.flistx && event.x < (config.skin.flistx+config.skin.flistw) ) {
            /* page down */
            if ( current[depth]+(config.skin.flistlines-1) > fl_maxpos ) {
                screenmark[depth] = 0;
                current[depth] = fl_maxpos;
