...
unsigned char rcpressed(void)
{
    ...
    if ( in == config.rc.play )
        return 1;
    else if ( in == config.rc.skipf )
        return 2;
    else if ( in == config.rc.skipb )
        return 3;
    else if ( in == config.rc.stop )
        return 4;
    else if ( in == config.rc.voldec )
        return 5;
    else if ( in == config.rc.volinc )
        return 6;
    else if ( in == config.rc.pause )
        return 7;
    else
        return 0;
