        ...
        if ( select(selval, &fds, NULL, NULL, &counter) > 0 ) {
            ...
            if ( FD_ISSET(0, &fds) ) {
                ...
                if ( ch == 3 && canexit() ) {
                    ...
                    exit(0);
                } /* ^C */ else if ( ch == 26 && canexit() ) disappear();
                else /* ^Z */
                    if ( ch == 12 ) { /* ^L */
                        ...
                        if (ch == '+') set_volume(config.voldev, config.volstep);
                        else if (ch == '-') set_volume(config.voldev, -config.volstep);
                        else
#endif
                            if ( ch == 27 ) {
                                ...
                                if ( ch2 == 'D' && pl_buttonpos != PL_MINBUTTON ) {
                                ...
                                } else if ( ch2 == 'C' && pl_buttonpos != PL_MAXBUTTON ) {
                                    pl_updatebuttons(1); /* right arrow */
                                } else if ( ch2 == 'B' && pl_current != pl_maxpos ) {
                                    if ( pl_screenmark != (config.skin.plistlines-1) ) /* down arrow */
                                        pl_screenmark++;
                                    pl_current++;
                                    pl_showents(pl_current-pl_screenmark, *playlist,filenumber);
                                    findit(NULL,0);
                                } else if ( ch2 == 'A' && pl_current != 0 ) {
                                    if ( pl_screenmark != 0 ) /* up arrow */
                                        pl_screenmark--;
                                    pl_current--;
                                    pl_showents(pl_current-pl_screenmark, *playlist,filenumber);
                                    findit(NULL,0);
                                } else if ( ch2 == keys[KEY_PGUP] && pl_current != 0  && *playlist ) { /* page up */
                                    ...
                                    pl_showents(pl_current-pl_screenmark, *playlist,filenumber);
                                    findit(NULL,0);
                                } else if ( ch2 == keys[KEY_PGDN] && pl_current != pl_maxpos && *playlist ) { /* page down */
                                    ...
                                    pl_showents(pl_current-pl_screenmark, *playlist,filenumber);
                                    findit(NULL,0);
                                } else if ( ch2 == keys[KEY_HOME] && pl_current != 0 && *playlist ) { /* home */
                                    pl_current = 0;
                                    pl_screenmark = 0;
                                    pl_showents(0, *playlist,filenumber);
                                    findit(NULL,0);
                                } else if ( ch2 == keys[KEY_END] && pl_current != pl_maxpos && *playlist ) { /* end */
                                    pl_current = pl_maxpos;
                                    pl_screenmark = 0;
                                    pl_showents(pl_current-pl_screenmark, *playlist,filenumber);
                                    findit(NULL,0);
...
void pl_search(char ch, struct playlistent *playlist, unsigned int *filenumber)
{
    int finish_playlist;
...
// int count=0;
    ...
    while ( playlist != NULL && playlist->next != NULL ) {
        if ( playlist->number == pl_current )
            printf("\e[%sm", config.skin.plistca);
        else if (playlist->number == *filenumber && playsong && config.skin.plistcc )
            printf("\e[%sm", config.skin.plistcc);
        else
            printf("\e[%sm", config.skin.plistci);
        ...
        if ( config.showtime > 1 ) {
            if ( playlist->length ) sprintf(shortname, "[%02lu:%02lu] ", playlist->length / 60, playlist->length % 60 );
            else if ( !strncasecmp(playlist->showname, "http://", 7) ) strcpy(shortname, "[strem] ");
            else strcpy(shortname, "[ n/a ] ");
        }
...
void pl_updatebuttons(int add)
{
    ...
    for (i=PL_MINBUTTON; i<(PL_MAXBUTTON+1); i++) {
        ...
        if (config.skin.plistbo[pl_buttonpos] == i) printf("%s", config.skin.pa[i]);
        else
...
void pl_dofunction(struct playlistent **playlist, unsigned int *filenumber, int forcedbutton)
{
    ...
    case 0: /* browse */
        ...
        pl_maxpos = pl_count(*playlist)-1;
        if ( *playlist == NULL )
            pl_maxpos = pl_screenmark = pl_current = 0;
        else if (pl_current > pl_maxpos)
...
//      if ( length != (*playlist)->length )
    ...
    case 2: /* remove current entry */
        ...
        pl_seek(pl_current, playlist);
        if ( (*playlist)->next ) temp = (*playlist)->next;
        else if ( (*playlist)->prev ) temp = (*playlist)->prev;
        else
    ...
    if ( (event->buttons & 4) && (event->type & GPM_DOWN) && !(event->type & GPM_DRAG) ) {
        if ( event->y == config.skin.plisty-1 && event->x >= config.skin.plistx && event->x < (config.skin.plistx+config.skin.plistw) ) {
            ...
            if ( pl_current < pl_screenmark )
                pl_current = pl_screenmark;
            pl_showents(pl_current-pl_screenmark, *playlist,filenumber);
        } else if ( event->y == config.skin.plisty+config.skin.plistlines && event->x >= config.skin.plistx && event->x < (config.skin.plistx+config.skin.plistw) ) {
            /* page down */
            if ( pl_current+(config.skin.plistlines-1) > pl_maxpos ) {
                pl_current = pl_maxpos;
                pl_screenmark = 0;
...
//    size_t filesize;
    ...
    if ( !showname ) {
        if ( scanid3 && getmp3info(filename, &mode, &samplerate, &bitrate, name, artist, NULL, NULL, NULL, NULL) && artist[0]+name[0] ) {
            ...
            strncpy((*playlist)->showname, buf, 99);
        } else if ( !strncasecmp(filename, "http://", 7) ) {
            strncpy( (*playlist)->showname, filename, 99 );
            ...
            for(j=0; (j<strlen(buf) && cspos<99) ; j++) {
                ...
                if ( buf[j] == '.' || buf[j] == '_' ) (*playlist)->showname[cspos] = 32;
                else if ( buf[j] == '-' && j != 0 && j+1 < strlen(buf) && buf[j-1] != ' ' && buf[j+1] != ' ' ) {
        ...
        while ( (*playlist)->next != NULL && *playlist != NULL ) {
            ...
            for(j=0; j<strlen((*playlist)->showname); j++)
                ...
                if( tolower((*playlist)->showname[j]) < tolower(temp->showname[j]) ) {
                    temp = *playlist;
                    break;
                } else if ( tolower((*playlist)->showname[j]) != tolower(temp->showname[j]) )
                    break;
            *playlist = (*playlist)->next;
        }
...
{
    if ( text != NULL )
        printf("\e[%d;%dH\e[%sm%s\e[%d;%dH%s", config.skin.texty, config.skin.textx, config.skin.textc, xys(config.skin.textw, ' '), config.skin.texty, config.skin.textx, text);
    else
